var randomData = require('../randomData/index.js');

var mockData = {};
mockData.data = randomData;//随机数
...
mockData.random = true;
mockData.generateFromTemplate = function (template, name) {

    var length = 0;
    var matches = (name || '').match(/\w+\|(\d+)-(\d+)|\w+\|(\d+)/);//匹配 |length 或 | min-max;
    if (matches) {
        if (matches[2]) { //| min-max
    ...
    switch (type(template)) {
        ...
        case 'object':
            ...
            for (var p in template) {
                ...
                generated[p.replace(/\|(\d+-\d+|\+\d+|\d+)/, '')] = mockData.generateFromTemplate(template[p], p);
            }
            break;
...
function getRandomData(key) {
    ...
    if (!(key in mockData.data)) {
        return key;
    }

    var a = mockData.data[key];
